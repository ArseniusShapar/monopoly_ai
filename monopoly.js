...
function log_inf(player_name, action, args) {
    var str = `${idx} | ${player_name} | ${action} | ${JSON.stringify(args)}`
    console.log(str);
	idx += 1;
}
...
function chanceAction(chanceIndex) {
	...
	log_inf(p.name, 'chance', {'card': chanceCards[chanceIndex].text});
}
...
function communityChestAction(communityChestIndex) {
	...
	log_inf(p.name, 'community_chest', {'card': communityChestCards[communityChestIndex].text});
}
...
function addamount(amount, cause) {
	...
	log_inf(p.name, 'add_amount', {'amount': amount, 'cause': cause});
}
...
function subtractamount(amount, cause) {
	...
	log_inf(p.name, 'subtract_amount', {'amount': amount, 'cause': cause});
}
...
function buyHouse(index) {
	...
	log_inf(p.name, 'buy_house', {'square': sq.name});
}
...
function sellHouse(index) {
	...
	log_inf(p.name, 'sell_house', {'square': sq.name});
}
...
function buy() {
	...
	log_inf(p.name, 'buy_property', {'property': property.name});
}
...
function mortgage(index) {
	...
	log_inf(p.name, 'mortgage', {'square': sq.name});
...
function unmortgage(index) {
	...
	log_inf(p.name, 'unmortgage', {'square': sq.name});
...
function land(increasedRent) {
	...
	log_inf(p.name, 'land', {'square': s.name, 'position': p.position});
}
...
function roll() {
	...
	// Это ненормально, желатьно убрать
	// if (die1 == die2) {
	// 	if (die2 == 6) {
	// 		die2 -= 1;
	// 	} else {
	...
	// sq = square[p.position + die1 + die2]
